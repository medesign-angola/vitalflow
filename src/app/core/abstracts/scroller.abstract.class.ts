import { AfterViewInit, computed, Directive, ElementRef, HostListener, input, linkedSignal, model, OnChanges, output, signal, SimpleChanges, WritableSignal } from "@angular/core";
...
export abstract class Scroller<T = any> implements AfterViewInit, OnChanges{
    ...
    paddingX = linkedSignal(() => this.limitedContainerElementRef()?.nativeElement.offsetLeft ?? 0, );
    ...
    protected valueToPadding(){
        this.paddingX.set(this.limitedContainerElementRef()?.nativeElement.offsetLeft ?? 0);
    }
